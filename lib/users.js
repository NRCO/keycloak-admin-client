...
/**
  A function to impersonate a user session
  @param {string} realmName - The name of the realm(not the realmID) - ex: master
  @param {string} userId - Use this to identify which user to impersonate
  @returns {Promise} A promise that will resolve with a session
  @example
  keycloakAdminClient(settings)
    .then((client) => {
      client.users.impersonate(userId)
        .then((session) => {
        console.log(session) // [{...},{...}, ...]
      })
    })
 */
function impersonate (client) {
  return function impersonate (realm, userId) {
    return new Promise((resolve, reject) => {
      const req = {
        auth: {
          bearer: privates.get(client).accessToken
      ...
      req.url = `${client.baseUrl}/admin/realms/${realm}/users/${userId}/impersonation`;
